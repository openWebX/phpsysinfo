...
class PingTest extends PSI_Plugin {
    ...
    public function __construct ($enc) {
        ...
        if (defined('PSI_PLUGIN_PINGTEST_ADDRESSES') && is_string(PSI_PLUGIN_PINGTEST_ADDRESSES)) {
            if (preg_match(ARRAY_EXP, PSI_PLUGIN_PINGTEST_ADDRESSES)) {
                ...
                $addresses = [PSI_PLUGIN_PINGTEST_ADDRESSES];
            }
                ...
                case 'command':
                    ...
                    foreach ($addresses as $address) {
                        ...
                        if ((strlen($buffer) > 0) && preg_match("/ time[=<]([\d\.]+)\s*ms/", $buffer, $tmpout)) {
                            $this->_filecontent[] = [$address, $tmpout[1]];
                        }
                    }
                    break;
                case 'data':
                    ...
                    foreach ($addresses as $address) {
                        ...
                        if (count($pt) == 2) {
                            $this->_filecontent[] = [trim($pt[0]), trim($pt[1])];
                        }
                    }
                    break;
                default:
    ...
    public function execute () {
        if (defined('PSI_PLUGIN_PINGTEST_ADDRESSES') && is_string(PSI_PLUGIN_PINGTEST_ADDRESSES)) {
            ...
            foreach ($addresses as $address) {
                $this->_result[] = [$address, $this->address_inarray($address, $this->_filecontent)];
            }
        }
    }
