        ...
        switch (strtolower(PSI_PLUGIN_BAT_ACCESS)) {
            case 'command':
                if (PSI_OS == 'WINNT') {
                    ...
                    $buffer_info = '';
                    $buffer_state = '';
                    $bufferWB = CommonFunctions::getWMI($_cim, 'Win32_Battery', ['EstimatedChargeRemaining', 'DesignVoltage', 'BatteryStatus', 'Chemistry']);
                    if (sizeof($bufferWB) > 0) {
                        ...
                        $bufferWPB = CommonFunctions::getWMI($_cim, 'Win32_PortableBattery', ['DesignVoltage', 'Chemistry', 'DesignCapacity', 'FullChargeCapacity']);
                        ...
                        $bufferBS = CommonFunctions::getWMI($_wmi, 'BatteryStatus', ['RemainingCapacity', 'Voltage']);
                        ...
                        if (!isset($bufferWPB[0]['FullChargeCapacity'])) {
                            $bufferBFCC = CommonFunctions::getWMI($_wmi, 'BatteryFullChargedCapacity', ['FullChargedCapacity']);
                        ...
                        $bufferBCC = CommonFunctions::getWMI($_wmi, 'BatteryCycleCount', ['CycleCount']);
