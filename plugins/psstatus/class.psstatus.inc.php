                ...
                case 'command':
                    if (PSI_OS == 'WINNT') {
                        try {
                            ...
                            $process_wmi = CommonFunctions::getWMI($wmi, 'Win32_Process', ['Caption', 'ProcessId']);
                            foreach ($process_wmi as $process) {
                                $this->_filecontent[] = [strtolower(trim($process['Caption'])), trim($process['ProcessId'])];
                            }
                        } catch (Exception $e) {
                        }
                    } else {
                        ...
                        if (defined('PSI_PLUGIN_PSSTATUS_USE_REGEX') && PSI_PLUGIN_PSSTATUS_USE_REGEX === true) {
                            foreach ($processes as $process) {
                                ...
                                if (strlen($buffer) > 0) {
                                    $this->_filecontent[] = [$process, $buffer];
                                }
                            }
                        } else {
                            foreach ($processes as $process) {
    ...
    public function execute () {
        if (defined('PSI_PLUGIN_PSSTATUS_PROCESSES') && is_string(PSI_PLUGIN_PSSTATUS_PROCESSES)) {
            ...
            if ((PSI_OS == 'WINNT') && (strtolower(PSI_PLUGIN_PSSTATUS_ACCESS) == 'command')) {
                foreach ($processes as $process) {
                    $this->_result[] = [$process, $this->process_inarray(strtolower($process), $this->_filecontent)];
                }
            } else {
                foreach ($processes as $process) {
                    $this->_result[] = [$process, $this->process_inarray($process, $this->_filecontent)];
                }
            }
        }
    }
