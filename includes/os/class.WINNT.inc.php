...
class WINNT extends OS {
    ...
    private function _get_Win32_OperatingSystem () {
        if ($this->_Win32_OperatingSystem === null) $this->_Win32_OperatingSystem = CommonFunctions::getWMI($this->_wmi, 'Win32_OperatingSystem', ['CodeSet', 'OSLanguage', 'LastBootUpTime', 'LocalDateTime', 'Version', 'ServicePackMajorVersion', 'Caption', 'OSArchitecture', 'TotalVisibleMemorySize', 'FreePhysicalMemory']);
    ...
    private function _get_Win32_Processor () {
        if ($this->_Win32_Processor === null) $this->_Win32_Processor = CommonFunctions::getWMI($this->_wmi, 'Win32_Processor', ['LoadPercentage', 'AddressWidth', 'Name', 'L2CacheSize', 'L3CacheSize', 'CurrentClockSpeed', 'ExtClock', 'NumberOfCores', 'NumberOfLogicalProcessors', 'MaxClockSpeed']);
    ...
    private function _get_Win32_ComputerSystem () {
        if ($this->_Win32_ComputerSystem === null) $this->_Win32_ComputerSystem = CommonFunctions::getWMI($this->_wmi, 'Win32_ComputerSystem', ['Name', 'Manufacturer', 'Model']);
    ...
    protected function _users () {
        if (CommonFunctions::executeProgram('quser', '', $strBuf, false) && (strlen($strBuf) > 0)) {
            ...
            $users = 0;
            $buffer = CommonFunctions::getWMI($this->_wmi, 'Win32_Process', ['Caption']);
    ...
    private function _get_Win32_PerfFormattedData_PerfOS_Processor () {
        if ($this->_Win32_PerfFormattedData_PerfOS_Processor === null) {
            $cpubuffer = CommonFunctions::getWMI($this->_wmi, 'Win32_PerfFormattedData_PerfOS_Processor', ['Name', 'PercentProcessorTime']);
            $this->_Win32_PerfFormattedData_PerfOS_Processor = [];
    ...
    private function _network () {
        if ($this->_wmi) {
            ...
            if ($buffer && isset($buffer[0]) && isset($buffer[0]['Version']) && preg_match('/^(\d+)\.(\d+)/', $buffer[0]['Version'], $version)
                ...
                $allDevices = CommonFunctions::getWMI($this->_wmi, 'Win32_PerfRawData_Tcpip_NetworkAdapter', ['Name', 'BytesSentPersec', 'BytesTotalPersec', 'BytesReceivedPersec', 'PacketsReceivedErrors', 'PacketsReceivedDiscarded', 'CurrentBandwidth']);
            } else {
                $allDevices = CommonFunctions::getWMI($this->_wmi, 'Win32_PerfRawData_Tcpip_NetworkInterface', ['Name', 'BytesSentPersec', 'BytesTotalPersec', 'BytesReceivedPersec', 'PacketsReceivedErrors', 'PacketsReceivedDiscarded', 'CurrentBandwidth']);
            }
            ...
            if ($allDevices) {
                $aliases = [];
                $allAdapters = CommonFunctions::getWMI($this->_wmi, 'Win32_NetworkAdapter', ['Name', 'GUID']);
                foreach ($allAdapters as $adapter) {
                    if (isset($adapter['GUID'])) {
                        $cname = str_replace(['(', ')', '#'], ['[', ']', '_'], $adapter['Name']); //convert to canonical
                ...
                $allNetworkAdapterConfigurations = CommonFunctions::getWMI($this->_wmi, 'Win32_NetworkAdapterConfiguration', ['Description', 'MACAddress', 'IPAddress', 'SettingID']);
                foreach ($allDevices as $device) {
                    ...
                    if ($dev->getName() == "") { //no alias or no alias description
                        $cname = str_replace(['(', ')', '#'], ['[', ']', '_'], $name); //convert to canonical
                        ...
                        foreach ($allNetworkAdapterConfigurations as $NetworkAdapterConfiguration) {
                            if (str_replace(['(', ')', '#'], ['[', ']', '_'], $NetworkAdapterConfiguration['Description']) === $cname) {
    ...
    private function _devicelist ($strType) {
        if (empty($this->_wmidevices)) {
            $this->_wmidevices = CommonFunctions::getWMI($this->_wmi, 'Win32_PnPEntity', ['Name', 'PNPDeviceID']);
        }
        $list = [];
    ...
    private function _filesystems () {
        $typearray = ['Unknown', 'No Root Directory', 'Removable Disk', 'Local Disk', 'Network Drive', 'Compact Disc', 'RAM Disk'];
        $floppyarray = ['Unknown', '5 1/4 in.', '3 1/2 in.', '3 1/2 in.', '3 1/2 in.', '3 1/2 in.', '5 1/4 in.', '5 1/4 in.', '5 1/4 in.', '5 1/4 in.', '5 1/4 in.', 'Other', 'HD', '3 1/2 in.', '3 1/2 in.', '5 1/4 in.', '5 1/4 in.', '3 1/2 in.', '3 1/2 in.', '5 1/4 in.', '3 1/2 in.', '3 1/2 in.', '8 in.'];
        $buffer = CommonFunctions::getWMI($this->_wmi, 'Win32_LogicalDisk', ['Name', 'Size', 'FreeSpace', 'FileSystem', 'DriveType', 'MediaType']);
